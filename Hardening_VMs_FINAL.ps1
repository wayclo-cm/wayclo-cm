    ...
    foreach ($key in $settings.Keys) {
        ...
        if ($currentSetting) {
            ...
            if ($status -eq "Warning") {
                Write-Host "ALERTA: Configuración '$key' en la VM '$($vm.Name)' tiene el valor '$currentValue'."
            } else {
            ...
            Write-Host "ALERTA: Configuración '$key' no existe en la VM '$($vm.Name)'."
        }
...
<html>
<head>
    <title>Verificación de Configuraciones Avanzadas de VMs</title>
    <style>
